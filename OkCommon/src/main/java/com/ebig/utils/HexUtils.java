...
public class HexUtils {
    ...
    public static String makeReciveHex(String order) {
        int i = HexUtils.hex2int(order);
        return HexUtils.int2Hex(i | 0x80);
    }
    ...
    public static String getFingerId(int pid, int gid) {
        return long2HexReverse(pid) + int2Hex(gid);

    }

    /*获取异或校验和*/
    public static String getXORCheck(String hex) {
        List<Integer> list = StrUtils.spiltHexInt(hex);
        int first = list.get(0);
        for (int i = 1; i < list.size(); i++) {
            first ^= list.get(i);
        }
        //  return String.format("%x", (int)x);
        return int2Hex(first);
    }
