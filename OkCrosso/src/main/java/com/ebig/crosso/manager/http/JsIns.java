...
public class JsIns {
    ...
    private static class JsInsHolder {
        private static JsIns jsIns = new JsIns();
    }

    public static JsIns L() {
        return JsInsHolder.jsIns;
    }
     ...
     */
    public final String EMPTY_JSON_ARRAY = "[]";

    /**
     * 默认的 {@code JSON} 日期/时间字段的格式化模式。
     */
    public final String DEFAULT_DATE_PATTERN = "yyyy-MM-dd HH:mm:ss SSS";
     ...
     * 注解常用的版本号常量 - {@code 1.0}。
     */
    public final double SINCE_VERSION_10 = 1.0d;
     ...
     * 注解常用的版本号常量 - {@code 1.1}。
     */
    public final double SINCE_VERSION_11 = 1.1d;
     ...
     * 注解常用的版本号常量 - {@code 1.2}。
     */
    public final double SINCE_VERSION_12 = 1.2d;
     ...
     * 注解常用的版本号常量 - {@code 1.0}。
     */
    public final double UNTIL_VERSION_10 = SINCE_VERSION_10;
     ...
     * 注解常用的版本号常量 - {@code 1.1}。
     */
    public final double UNTIL_VERSION_11 = SINCE_VERSION_11;
     ...
     * 注解常用的版本号常量 - {@code 1.2}。
     */
    public final double UNTIL_VERSION_12 = SINCE_VERSION_12;
     ...
     * @since 1.0
     */
    public String toJson(Object target, Type targetType, boolean isSerializeNulls, Double version, String datePattern, boolean excludesFieldsWithoutExpose) {
    ...
    public <T> List<T> fromJsonArray(String json, Class<T> clazz) {
        ...
        try {
            ...
            for (int i = 0; i < array.length(); i++) {
                ...
                T entity = fromJson(obj.toString(), clazz);
                list.add(entity);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public <T> List<T> fromJsonArray(String json, Class<T> clazz, CommonCall<Exception> callBack) {
        List<T> list = new ArrayList<>();
        try {
            JSONArray array = new JSONArray(json);
            for (int i = 0; i < array.length(); i++) {
    ...
    public JSONObject getJSONObject(JSONObject object, String key) {
        JSONObject sencodObj = null;
        try {
            sencodObj = object.getJSONObject(key);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return sencodObj;
    ...
    public String getJson(String name, Object value) {
        String json = "";
        try {
            json = new JSONObject().put(name, value).toString();
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return json;
    ...
    public String getJson(List<KeyValue> list) {
        JSONObject object = new JSONObject();
        try {
            for (int i = 0; i < list.size(); i++) {
                object.put(list.get(i).getKey(), list.get(i).getValue());
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public <T> List<T> getList(String json, final T t) {
        //  List<RsNewsSummary> list= new Gson().fromJson(bean.getJson(),new TypeToken<List<RsNewsSummary>>(){}.getType());
